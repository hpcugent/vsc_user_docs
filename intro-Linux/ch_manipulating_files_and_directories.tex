...
Here we used the same \lstinline|cp| command, but instead we gave it the \lstinline|-a| option
...
Any time you run \lstinline|ls -l| you'll see a familiar line of \lstinline|-rwx------|
or similar combination of the letters \lstinline|r|, \lstinline|w|, \lstinline|x| and \lstinline|-| (dashes).
These are the permissions for the file or directory. (See also the
\hyperref[sec:permissions]{previous section on permissions})
...
Here, we see that \lstinline|articleTable.csv| is a file (beginning the line with \lstinline|-|)
has read and write permission for the user \texttt{\userid} (\lstinline|rw-|), and read
permission for the group \lstinline|mygroup| as well as all other users (\lstinline|r--| and
\lstinline|r--|).

The next entry is \lstinline|Project_GoldenDragon|. We see it is a directory because the
...
The syntax used here is \lstinline|g+x| which means \strong{g}roup was given \strong{w}rite
...
This will give the \strong{u}ser \lstinline|otheruser| permissions to \strong{w}rite
...
If you use \lstinline|tar| with the source files first then the first file will be
