...
Here we used the same \verb|cp| command, but instead we gave it the \verb|-a| option
...
Any time you run \verb|ls -l| you'll see a familiar line of \verb|-rwx------|
or similar combination of the letters \verb|r|, \verb|w|, \verb|x| and \verb|-| (dashes).
...
Here, we see that \verb|articleTable.csv| is a file (beginning the line with \verb|-|)
has read and write permission for the user \texttt{\userid} (\verb|rw-|), and read
permission for the group \verb|mygroup| as well as all other users (\verb|r--| and
\verb|r--|).

The next entry is \verb|Project_GoldenDragon|. We see it is a directory because the
...
The syntax used here is \verb|g+x| which means \strong{g}roup was given \strong{w}rite
...
This will give the \strong{u}ser \verb|otheruser| permissions to \strong{w}rite
