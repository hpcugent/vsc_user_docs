...
Alternatively you can use the command shown by the warning under
\lstinline|remove with:| that should be something like this:

\begin{prompt}
sh-keygen -f "~/.ssh/known_hosts" -R "%\loginnode{}%"
\end{prompt}
...
\strong{If you get a message ``Host key for server changed'', do not click ``No'' until you verified the fingerprint.}.
