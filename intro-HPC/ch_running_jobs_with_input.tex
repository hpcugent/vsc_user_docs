...
\$VSC\_SCRATCH\_VO      & The scratch filesystem for your VO storage, this can have a bigger quota then your personal scratch.\\ \hline
\fi
\end{tabular}
...
larger than a few megabytes), and preferably is should only contain configuration
files where the user has no direct control over where they are supossed to be found.
Also note that various kinds of configuration files are also stored
...
We can give higher quota values for these filesystems, up to 100TiB is no exception.
\fi
...
are in a VO. This is extra SCRATCH space you can request if you are running
...
\ifgent
To see your current quota usage you can run the ``\emph{show_quota}``` script
on the \hpcInfra.
...
However, we urge you to use the \texttt{du} and \texttt{tree} commands only when you
