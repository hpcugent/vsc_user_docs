...
\ifbrussel
  ...
  \url{https://www.vscentrum.be/infrastructure/hardware/hardware-vub}.
\fi
\ifgent
...
include one or more sockets, each housing a multi-core processor (next to
memory, disk(s), network cards, ...). A modern processor consists of
multiple CPUs or cores that are used to execute \emph{computations}.
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item Move your results from the cluster to your computer.
\end{enumerate}
