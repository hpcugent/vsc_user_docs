...
\$VSC\_SCRATCH\_ARCANINE  & A separate (smaller) shared scratch filesystem, powered by SSDs. This scratch filesystem is intended for very I/O-intensive workloads. Note that this filesystem is not mounted on the cluster victini.\\ \hline
\fi
\end{tabular}
...
\begin{tip}
\ifleuven\else\ifantwerpen\else % VOs
...
\fi\fi
\ifgent\else\ifbrussel\else % no VOs
...
% virtual organisations are only used in Ghent and Brussel (for now)
