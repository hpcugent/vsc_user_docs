...
\ifgent
Note: \verb|$VSC_SCRATCH_KYUKON| and \verb|$VSC_SCRATCH| are the same directories.
...
\begin{enumerate}
    ...
    \item Fill out the both the internal and public VO name. These cannot contain
        spaces, and should be 8-10 characters long. For example, \verb|genome25| is
        a valid VO name.
...
\begin{enumerate}
    ...
    \item Fill out how much \emph{additional} storage you want. In the screenshot below,
        we're asking for 500 GiB extra space for \verb|VSC_DATA|, and for 1 TiB extra
        space on \verb|VSC_SCRATCH_KYUKON|.
...
When you're in a VO, there will be some extra filesystems available (these
filesystems are only available if your are member of a VO):

\begin{description}
    \item[VO scratch (\texttt{\$VSC\_SCRATCH\_*\_VO})]
        This is extra scratch space you can use if you are running
        out of quota on your personal scratch. These include:
        \begin{enumerate}
            \item VSC_SCRATCH_DELCATTY_VO
            \item VSC_SCRATCH_KYUKON_VO
            \item VSC_SCRATCH_PHANPY
        \end{enumerate}

    \item[VO data (\texttt{\$VSC\_DATA\_VO})]
        This is an extra filesystem for datasets, \ldots you can use if you are running
        out of quota on your personal data filesystem (\verb|$VSC_DATA|).

\end{description}

If you put \verb|_USER| after each of these variable names, you can see your
personal folder in these filesystems. For example: \verb|$VSC_DATA_VO_USER|
is your personal folder in your VO data filesystem.
