...
We can give higher quota values for these filesystems. VO administrators can request additional quota
at the "Edit VO" page \url{https://account.vscentrum.be/django/vo/edit}.

Note that we may oversubscribe each shared filesystem, meaning that the total amount of quota across
all VO's may exceed the total size of the filesystem. Because of this, you should consider the available quota
as an upper limit of the amount of storage space that is available to you; you may not be able to use it all
if the filesystem does not provide a sufficient amount of free space.
...
A Virtual Organisation (VO) is a sort of group. You can either belong to no VO
or to a single VO. Being in a VO allows for larger storage quota to be obtained
...
\begin{enumerate}
    \item Get the VO id of the research group you belong to (this id is formed by
        the letters \verb|gvo|, followed by 5 digits).
    \item Go to \url{https://account.vscentrum.be/django/vo/join} and fill in the
        ...
        the moderator of the VO, where you identify yourself.
    \item A message will be sent to the moderator of the VO, who will either approve
        or deny the request.
\end{enumerate}
...
\subsection{Request more storage space}

If you're a moderator of a VO, you can request additional quota.

\begin{enumerate}
    \item Go to \url{https://account.vscentrum.be/django/vo/edit} and scroll down
        to ``Request additional quota''.
    \item Fill out how much \emph{additional} storage you want.
    \item Add a comment explaing why you need additional storage space and submit
        the form.
