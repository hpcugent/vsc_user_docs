...
\begin{tabular}{|p{\dimexpr 0.18\textwidth-2\tabcolsep}|p{\dimexpr 0.27\textwidth-2\tabcolsep}|p{\dimexpr 0.55\textwidth-2\tabcolsep}|} \hline
...
Scoop                                                                  & Python                                                                 & Applies to both shared and distributed memory system. Not extremely advanced, but may present a quick road to parallelisation of Python code.  \\ \hline
\end{tabular}
...
a wrapper around \lstinline|mpirun| called \lstinline|mympirun|. See \autoref{ch:mympirun} for
more information.

You will generally just start an MPI program on the \hpcInfra by using \lstinline|mympirun|
instead of \lstinline|mpirun -n <nr of cores> <--other settings> <--other optimisations>|
\end{tip}
\fi
