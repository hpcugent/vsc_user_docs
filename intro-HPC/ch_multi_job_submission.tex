...
First go to the right directory:
\begin{prompt}
%\shellcmd{cd \tilde/examples/Chapter08\_MultiJob/job\_array}%
\end{prompt}

A typical job-script for use with job arrays would look like this:
\begin{prompt}
%\shellcmd{cat job\_array.pbs}%
#!/bin/bash -l
#PBS -l nodes=1:ppn=1
#PBS -l walltime=00:15:00
cd $PBS_O_WORKDIR
...
\begin{prompt}
%\shellcmd{cd \tilde/examples/Chapter08\_MultiJob/map\_reduce}%
\end{prompt}
...
\begin{prompt}
%\shellcmd{wsub -prolog pre.sh -batch test\_set.pbs -epilog post.sh -t 1-100}%
total number of work items: 100
%\jobid%
...
running example, this could be 'run.pbs.log445948', assuming the job's ID is
