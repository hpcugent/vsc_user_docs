...
\begin{enumerate}
\item  Activates or deactivates \underbar{software packages} and their dependencies.
...
E.g., \lstinline|foss-2014a| is the first version of the \lstinline|foss| toolchain in 2014.
...
\begin{prompt}
%\shellcmd{module load \emph{examplemodule}}%
\end{prompt}

This will load the most recent version of \emph{examplemodule}.
...
Currently Loaded Modulefiles:
  1) examplemodule/1.2.3-intel-2018a        3) intel/2018a
  2) secondexample/2.7-intel-2018a          4) examplelib/1.2-intel-2014b
\end{prompt}


It is important to note at this point that other modules (e.g., \lstinline|intel/2014a|)
...
modules manually, to debug some problem. When the \lstinline|secondexample| module is unloaded,
...
\begin{prompt}
%\shellcmd{module load \emph{examplemodule/1.2.3-intel-2018a} \emph{secondexample/2.7-intel-2018a}}%
\end{prompt}
...
Consider the following example: the user decides to use the \lstinline|examplemodule| module
and at that point in time, just a single version 1.2.3,
...
\begin{prompt}
%\shellcmd{module load \emph{examplemodule/1.2.3-intel-2018a}}%
\end{prompt}

Everything works fine, up to the point where a new version of \lstinline|examplemodule| is installed,
4.5.6 compiled for gcc. From then on, the user's load command will load the
...
example, to list the available versions of \lstinline|examplemodule|:

\begin{prompt}
%\shellcmd{module avail examplemodule/}%
examplemodule/1.2.3-intel-2018a     examplemodule/4.5.6-GCCcore-6.3.0
examplemodule/1.2.3-foss-2018a

\end{prompt}

Lets now generate a version conflict with the \lstinline|thirdexample| module, and see what
is happening.

\ifusinglmod
\begin{prompt}
%\shellcmd{module av thirdexample/}%
thirdexample/1.2.3
thirdexample/4.5.6
thirdexample/6.7.8
%\shellcmd{module load thirdexample/1.2.3}%
%\shellcmd{module load thirdexample/4.5.6}%
Lmod has detected the following error:  A different version of the 'thirdexample'
...
%\shellcmd{module load thirdexample/1.2.3}%
%\shellcmd{module load thirdexample/4.5.6}%
thirdexample/4.5.6(12):ERROR:150: Module 'thirdexample/4.5.6' conflicts with the currently loaded module(s) 'thirdexample/1.2.3'
thirdexample/4.5.6(12):ERROR:102: Tcl command execution failed: conflict thirdexample
%\shellcmd{module swap thirdexample/4.5.6}%
\end{prompt}
\fi
...
\begin{prompt}
%\shellcmd{module help examplemodule}%
----------- Module Specific Help for 'examplemodule/1.2.3-intel-2018a' ---------------------------

   This is a module that doesn't really exist. It can be used to do things. - Homepage: https://example.com/
\end{prompt}
