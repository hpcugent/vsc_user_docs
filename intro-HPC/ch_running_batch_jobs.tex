...
\ifleuven
\begin{prompt}
%\shellcmd{module load matlab/R2017b Python/2.7.6-foss-2014a}%
\end{prompt}
\else
\begin{prompt}
%\shellcmd{module load MATLAB/2018a Python/2.7.14-intel-2018a}%
\end{prompt}
\fi
...
Consider the following example: the user decides to use the libpng library
and at that point in time, just a single version 1.6.23,
compiled with Intel is installed on the cluster. The user loads the library
using:

\begin{prompt}
%\shellcmd{module load libpng}%
\end{prompt}

rather than

\begin{prompt}
%\shellcmd{module load libpng/1.6.23-foss-2016b}%
\end{prompt}

Everything works fine, up to the point where a new version of libpng is installed,
1.6.34 compiled for gcc. From then on, the user's load command will load the
latter version, rather than the intended one, which may lead to unexpected
problems.

To list the available versions, you can use the \verb|module avail| command. For
example, to list the available versions of the libpng library:

\begin{prompt}
%\shellcmd{module avail libpng/}%
libpng/1.2.58                libpng/1.6.28-GCCcore-6.3.0
libpng/1.6.23-foss-2016b     libpng/1.6.29-GCCcore-6.3.0
libpng/1.6.23-intel-2016b    libpng/1.6.32-GCCcore-6.4.0
libpng/1.6.24-foss-2016b     libpng/1.6.34-GCCcore-6.4.0
libpng/1.6.24-intel-2016b    libpng/1.6.34-GCCcore-7.3.0 (D)
...
As indicated in the output above, each \verb|cluster| module is a so-called sticky
module, i.e. it will not be unloaded when \verb|module purge| (see \autoref{subsec:purging-modules}) is used.
