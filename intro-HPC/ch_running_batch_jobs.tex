...
\ifgent
The default memory on a given HPC-UGent cluster is the ``usable memory per node'' divided by
the ``numbers of cores in a node'' multiplied by the requested processor core(s) (ppn).
Jobs get the default memory without specifying any memory,
and this is the recommended option. For ``usable memory per node'' and ``number of
