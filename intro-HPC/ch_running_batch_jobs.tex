...
\ifbrussel
and is only allowed on the node on which your job is running
\fi
...
(called ``walltime'') is really important. Here, the job requests a reservation of
2 hours, 30 minutes. As soon as the job starts to take more time,
...
The job requests a reservation of 4 GB of RAM memory. As soon as the job tries to use more memory,
it will be ``killed'' (terminated) by the job scheduler.  There is absolutly no
...
The job requests a reservation of 5 compute nodes with two cores on each node (ppn stands for
