...
\item Click \strong{Create Volume}.
  ...
  \item[Type] Select one of the following options:
    ...
    \begin{itemize}
    \item tripleo: This is the default option for the projects,
      it creates the volume in a regular hdd storage pool.
    \item fastpool (optional): It creates the volume in a high I/O
      throughput storage pool. Fastpool has its own storage quota
      and it is only accessible by project's request (contact
      \cloudinfo if you need more information).
    \end{itemize}
