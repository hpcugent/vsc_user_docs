...
\section{SSH key pairs}
OpenStack can automatically inject a public SSH key into an instance
when it is launched.  This gives anyone with the corresponding private
key admin access to the instance.  OpenStack, this feature is called
``key pair injection\footnote{OpenStack consistently refers to SSH
  credentials as ``key pairs'', but of course only the public keys are
  stored in the OpenStack environment, while the private key should be
  kept secure by the owner.}''.  To use key pair injection, the image
...
information, see section \ref{add-a-key-pair}.  For general
...
\strong{Note:} In the VSC cloud, public keys from your VSC account are
automatically available in your cloud projects, so VSC users can
immediately use one of their existing keys to access instances.  If
you want to give other parties SSH access to VM's, you must manage the
keys using some other method.  \strong{Never} upload SSH keys for
other users to your VSC account.

\strong{Note:} Every OpenStack user account has its own collection of
SSH keys.  To share a public key between multiple users of the same
project, each user needs to import it into their account.
