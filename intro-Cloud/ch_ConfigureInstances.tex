...
connect to or provide VSC services via VSC network and floating or dedicated 
project IPs. When an instance is created in \gls{OpenStack} and connected to 
the \_vm, \_nfs or \_vsc networks, it is automatically assigned a fixed 
IP address in that network (the predefined project IP in case of a project). 
This IP address is permanently associated with the instance until 
...
This is another basic port forwarding configuration example without patterns,
in this case just to connect via SSH using external port 52000 to a running VM
called testvm:
\begin{code}{}
[DEFAULT]
floatingip=193.190.85.40
...
id's of the rules you want to remove.

\strong{Note:} Please note that when you remove and reinstantiate a VM, OpenStack
also removes the port forwarding rules assigned to this VM automatically. If 
you intantiate again the same VM you should run \lstinline{neutron_port_forward}
