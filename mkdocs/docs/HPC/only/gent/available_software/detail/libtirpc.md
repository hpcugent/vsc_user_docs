---
hide:
  - toc
---

libtirpc
========

# Available modules


The overview below shows which libtirpc installations are available per HPC-UGent Tier-2 cluster, ordered based on software version (new to old).

To start using libtirpc, load one of these modules using a `module load` command like:

```shell
module load libtirpc/1.3.7-GCCcore-15.2.0
```

*(This data was automatically generated on {{modules_last_updated}})*

| |accelgor|doduo|donphan|gallade|joltik|litleo|shinx|skiddo|
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
|libtirpc/1.3.7-GCCcore-15.2.0|x|x|x|x|x|x|x|x|
|libtirpc/1.3.6-GCCcore-14.3.0|x|x|x|x|x|x|x|x|
|libtirpc/1.3.6-GCCcore-14.2.0|x|x|x|x|x|x|x|x|
|libtirpc/1.3.5-GCCcore-13.3.0|x|x|x|x|x|x|x|x|
|libtirpc/1.3.4-GCCcore-13.2.0|x|x|x|x|x|x|x|x|
|libtirpc/1.3.3-GCCcore-12.3.0|x|x|x|x|x|x|x|x|
