---
hide:
  - toc
---

json-c
======

# Available modules


The overview below shows which json-c installations are available per HPC-UGent Tier-2 cluster, ordered based on software version (new to old).

To start using json-c, load one of these modules using a `module load` command like:

```shell
module load json-c/0.18-GCCcore-15.2.0
```

*(This data was automatically generated on {{modules_last_updated}})*

| |accelgor|doduo|donphan|gallade|joltik|litleo|shinx|skiddo|
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
|json-c/0.18-GCCcore-15.2.0|x|x|x|x|x|x|x|x|
|json-c/0.18-GCCcore-14.3.0|x|x|x|x|x|x|x|x|
|json-c/0.18-GCCcore-14.2.0|x|x|x|x|x|x|x|x|
|json-c/0.17-GCCcore-13.3.0|x|x|x|x|x|x|x|x|
|json-c/0.17-GCCcore-13.2.0|x|x|x|x|x|x|x|x|
|json-c/0.16-GCCcore-12.3.0|x|x|x|x|x|x|x|x|
