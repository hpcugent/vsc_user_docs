---
hide:
  - toc
---

libjpeg-turbo
=============

# Available modules


The overview below shows which libjpeg-turbo installations are available per HPC-UGent Tier-2 cluster, ordered based on software version (new to old).

To start using libjpeg-turbo, load one of these modules using a `module load` command like:

```shell
module load libjpeg-turbo/3.1.4.1-GCCcore-15.2.0
```

*(This data was automatically generated on {{modules_last_updated}})*

| |accelgor|doduo|donphan|gallade|joltik|litleo|shinx|skiddo|
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
|libjpeg-turbo/3.1.4.1-GCCcore-15.2.0|x|x|x|x|x|x|x|x|
|libjpeg-turbo/3.1.1-GCCcore-14.3.0|x|x|x|x|x|x|x|x|
|libjpeg-turbo/3.1.0-GCCcore-14.2.0|x|x|x|x|x|x|x|x|
|libjpeg-turbo/3.0.1-GCCcore-13.3.0|x|x|x|x|x|x|x|x|
|libjpeg-turbo/3.0.1-GCCcore-13.2.0|x|x|x|x|x|x|x|x|
|libjpeg-turbo/2.1.5.1-GCCcore-12.3.0|x|x|x|x|x|x|x|x|
|libjpeg-turbo/2.1.4-GCCcore-12.2.0|-|-|-|-|-|x|x|-|
|libjpeg-turbo/2.1.3-GCCcore-11.3.0|-|-|-|-|-|x|x|-|
